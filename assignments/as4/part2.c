//Implementing a variable shared between a child process and its parent process and custom signal handler - Problem 2
...
/* When a process which has installed signal handlers forks, the child process inherits the signals.  All caught signals may be reset to their default action by a call to the execve() function; ~man signal
     */
...
int main(){
        ...
        if(pid==0){
                printf("enter executable's name\n");
                char s[100];
                scanf("%s",s);
